...
function toSubmission(){return {submittedAt:new Date().toISOString(),score:document.getElementById('score')?.textContent||'',profile:answers.profile,confidence:answers.confidence,knowledge:answers.knowledge,quiz:gradeKnowledge(),satisfaction:answers.satisfaction,suggestion:answers.suggestion};}
function loadSubmissions(){try{return JSON.parse(localStorage.getItem(SUBMIT_KEY)||'[]')}catch(e){return[]}}
function flatSubmission(s){const row={submittedAt:s.submittedAt,score:s.score,quiz_score:s.quiz?.score??'',quiz_total:s.quiz?.total??'',quiz_percent:s.quiz?.percent??'',age:s.profile?.age||'',year:s.profile?.year||'',gpa:s.profile?.gpa||'',medAdminExp:s.profile?.medAdminExp||'',wardExp:s.profile?.wardExp||'',suggestion:s.suggestion||''};surveyData.confidenceItems.forEach((_,i)=>row['confidence_'+(i+1)]=s.confidence?.[i]||'');surveyData.knowledgeQuestions.forEach((_,i)=>row['knowledge_'+(i+1)]=s.knowledge?.[i]||'');Object.entries(s.satisfaction||{}).forEach(([game,items])=>Object.entries(items||{}).forEach(([k,v])=>{row['satisfaction_'+game+'_'+(Number(k)+1)]=v;}));return row;}

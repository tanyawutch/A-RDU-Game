...
function missingRequired(){const missing=[];const p=answers.profile;['age','year','gpa','medAdminExp','wardExp'].forEach(k=>{if(!p[k])missing.push(profileLabel(k));});if(answeredCount(answers.confidence)<surveyData.confidenceItems.length)missing.push('ความมั่นใจ');if(answeredCount(answers.knowledge)<surveyData.knowledgeQuestions.length)missing.push('แบบทดสอบ');if(satisfactionCount()<surveyData.satisfactionGames.length*surveyData.satisfactionDimensions.length)missing.push('ความพึงพอใจ');return missing;}
...
function satisfactionCount(){return Object.values(answers.satisfaction||{}).reduce((sum,row)=>sum+answeredCount(row),0);}
...
window.showSurvey=showSurvey;window.renderSurvey=renderSurvey;document.addEventListener('DOMContentLoaded',()=>{const b=$('surveyBtn');if(b)b.onclick=showSurvey;});
})();

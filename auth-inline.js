...
const allowedDomains = cfg.allowedDomains || ['mfu.ac.th','lamduan.mfu.ac.th'];
let session = null;
let profile = null;
window.ARD_AUTH_ONLY_SURVEY = true;
window.ARD_AUTH = { client, isReady: ready, getSession:()=>session, getProfile:()=>profile, isAdmin:()=>isAdmin() };
...
function showPublicSurvey(){questionnaireUrl();setText('userEmail','');show('survey');if(window.renderSurvey)window.renderSurvey();}
...
function renderAuth(message){
  ...
  host.innerHTML='<div class="auth-card"><img src="assets/logo-nursing.png" alt="โลโก้"><div><p class="eyebrow">ARD Learning Game</p><h1>เข้าสู่ระบบเพื่อทำแบบสอบถาม</h1><p>สมัครและเข้าใช้งานได้ด้วยอีเมล @mfu.ac.th หรือ @lamduan.mfu.ac.th เท่านั้น</p></div><label>อีเมล<input id="authEmail" type="email" autocomplete="email" placeholder="name@mfu.ac.th"></label><label>รหัสผ่าน<div class="password-field"><input id="authPassword" type="password" autocomplete="current-password" placeholder="อย่างน้อย 6 ตัวอักษร"><button class="password-toggle" id="authPasswordToggle" type="button" aria-label="แสดงรหัสผ่าน">👁</button></div></label><div class="auth-actions"><button class="primary" id="loginBtn" type="button">เข้าสู่ระบบ</button><button class="secondary" id="signupBtn" type="button">สมัครสมาชิก</button></div><button class="secondary" id="publicSurveyBtn" type="button">ทำแบบสอบถามโดยไม่ต้องเข้าสู่ระบบ</button><p class="auth-status" id="authStatus">'+(message||'')+'</p>'+(ready?'':'<div class="survey-warning">ยังไม่ได้ตั้งค่า Supabase URL/Anon key ในไฟล์ supabase-config.js</div>')+'</div>';
  bindPasswordToggle('authPasswordToggle','authPassword'); $('loginBtn').onclick=()=>login(); $('signupBtn').onclick=()=>signup(); $('publicSurveyBtn').onclick=showPublicSurvey;
}
...
async function refresh(){
  ...
  questionnaireUrl(); show('survey'); if(window.renderSurvey)window.renderSurvey();
}
async function login(){
  const email=$('authEmail').value.trim().toLowerCase(), password=$('authPassword').value;
  if(!isAllowedEmail(email)){setText('authStatus','ใช้งานได้เฉพาะอีเมล @mfu.ac.th หรือ @lamduan.mfu.ac.th');return;}
...
async function signup(){
  const email=$('authEmail').value.trim().toLowerCase(), password=$('authPassword').value;
  if(!isAllowedEmail(email)){setText('authStatus','สมัครได้เฉพาะอีเมล @mfu.ac.th หรือ @lamduan.mfu.ac.th');return;}
...
window.showHome = function(){ if(isAdmin()){['auth','survey','play'].forEach(id=>$(id)?.classList.remove('active'));$('home')?.classList.add('active');} else {showPublicSurvey();} };
